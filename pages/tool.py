...
from data import get_database_connection
...
alpha = mpox_CONTRY_absolute_counts = mpox_testdata[
    mpox_testdata["property.name"] == "COUNTRY"
]["value_text"].value_counts()
betha = pd.DataFrame(
    data=np.array([alpha.index, alpha.values]), index=["COUNTRY", "OCCURRENCES"]
).T
